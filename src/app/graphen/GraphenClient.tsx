...
export default function GraphenClient() {
    const { t } = useLanguage();
        ...
        try {
            ...
            if (svg) {
                ...
                texts.forEach((textEl) => {
                    textEl.setAttribute("fill", "rgba(255, 255, 255, 0.8)");
                    (textEl as SVGTextElement).style.fontWeight = "500";
                });
    ...
    const addFunction = () => {
        ...
        setFunctions([
            ...functions,
            { id: Date.now(), expr: newFunc, visible: true, color: nextColor },
        ]);
        setNewFunc("");
    };
    ...
    const addPreset = (expr: string) => {
        ...
        setFunctions([
            ...functions,
            { id: Date.now(), expr, visible: true, color: nextColor },
        ]);
    };
                ...
                <div className="w-80 flex flex-col gap-4 h-full pr-6 border-r border-border/40">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-muted-foreground ml-auto">
                            {functions.filter((f) => f.visible).length} {t("common.active")}
                        </span>
                    </div>
                    ...
                    <div className="flex flex-col gap-1.5 flex-1 overflow-y-auto pr-1">
                        {functions.map((fn) => (
                            <div
                                key={fn.id}
                                className={`flex items-center gap-2 p-2.5 rounded-xl border transition-all ${fn.visible
                                        ? "bg-background shadow-sm border-border/80"
                                        : "bg-transparent border-dashed border-border/40 opacity-50"
                                    }`}
                            >
